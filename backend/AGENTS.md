...
- **Exact-source dispatch** — for an approved shared owner-Beta deployment, first require successful push-triggered Release Eligibility for its exact merged SHA. Replace the placeholder in `gh workflow run gcp_backend.yml --ref main -f environment=development -f mode=deploy -f release_sha='<approved-40-character-merged-SHA>'`. `branch` is not an input; another SHA's PR checks are not eligibility. `prod` requires separate approval/configuration.
