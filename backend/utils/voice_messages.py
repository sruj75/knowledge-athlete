...
from utils.request_validation import parse_voice_message_filename_timestamp
...
def _get_opus_decoder_class() -> Any:
    if Decoder is None:
        raise RuntimeError(
            'Opus voice-message decoding requires opuslib and the native libopus library. '
            'Install the OS-level Opus package before processing Opus voice messages.'
    ...
    try:
        with open(opus_file_path, 'rb') as f, wave.open(wav_file_path, 'wb') as wav_file:
            ...
            while True:
                ...
                if frame_length == 0 or frame_length > MAX_VOICE_FRAME_BYTES:
...
def get_timestamp_from_path(path: str) -> int | float:
    return parse_voice_message_filename_timestamp(path)


def retrieve_file_paths(files: List[UploadFile], uid: str) -> List[str]:
    directory = f'voice-messages/{uid}/'
...
def decode_pcm_file_to_wav(
    ...
    try:
        ...
        wav_data = _pcm_to_wav(bytes(pcm_data), sample_rate=sample_rate, channels=channels, sample_width=sample_width)
