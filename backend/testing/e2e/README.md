...
This version proves the backend can boot hermetically and that selected task/Memory CRUD, user/account, storage, retrieval/search, and legacy-shape paths can execute without real Firestore, Redis, GCS, Pinecone, Typesense, Google ADC, or production API keys. Conversation CRUD and processing are deliberately absent: the Mac owns its conversation archive locally. The authenticated transient listen route is exercised by `tests/unit/test_listen_transient_contract.py` with the real FastAPI route/runtime and a fake Modulate socket.
