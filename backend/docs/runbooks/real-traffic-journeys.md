...
`intentive_journey_*` and `intentive_live_stt_*` measure user-originated traffic only. They do not create test
...
| --- | --- | --- |
| `intentive_journey_accepted_total` | `journey` | A production boundary accepted work. |
| `intentive_journey_terminal_total` | `journey`, `outcome` | A one-shot terminal outcome for accepted work. |
| `intentive_journey_latency_seconds` | `journey`, `outcome` | Acceptance-to-terminal latency. |
| `intentive_live_stt_accepted_total` | `provider`, `client_platform`, `deployment_environment` | A listener accepted its first nontrivial live-STT audio frame. |
| `intentive_live_stt_terminal_total` | `provider`, `outcome`, `client_platform`, `deployment_environment`, `phase` | A one-shot terminal outcome for an accepted live-STT attempt. |
| `intentive_live_stt_terminal_failures_total` | `provider`, `outcome`, `client_platform`, `deployment_environment`, `phase` | Provider failure detail correlated with a live-STT terminal. |
| `account_deletion_telemetry` log event | bounded lifecycle fields | Admission, claim, terminal cleanup, and retry-safe durable deletion outcomes. |
...
- Live STT is accepted when `/v4/listen` receives its first
  ...
  provider outcome and phase in `intentive_live_stt_terminal_failures_total`; the
  matching accepted attempt has one `failure` terminal in
  `intentive_live_stt_terminal_total`.
