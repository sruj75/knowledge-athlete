...
The dashboard text panel repeats paging policy: page only on exhausted outcomes and the pusher degraded ratio. Successful `outcome=recovered` heals are dashboard-only.
...
|-----------|---------------|----------------|-----------|
| `audio_merge` | Cloud Tasks enqueue failure → inline merge on backend-sync | Logs only (`audio_merge:` prefix) | `record_fallback` on inline fallback plus a dedicated rate alert |
