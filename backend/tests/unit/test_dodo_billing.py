...
class _ActiveProvider:
    ...
    async def create_checkout(self, **kwargs):
        from utils.billing.provider import CheckoutSession
...
class _RetrieveResource:
    ...
    async def retrieve(self, identity: str) -> dict:
...
@pytest.mark.asyncio
async def test_first_subscription_retrieval_resolves_uid_from_checkout_payment_metadata() -> None:
    ...
    assert client.payments.calls == ['payment-synthetic']


@pytest.mark.asyncio
async def test_subscription_uid_avoids_first_payment_lookup() -> None:
    provider = object.__new__(DodoBillingProvider)
    client = _RetrieveClient(
        {
            'subscription_id': 'subscription-synthetic',
        ...
        {'payment_id': 'payment-synthetic', 'metadata': {'uid': 'wrong-user'}},
    )
    provider._client = client
