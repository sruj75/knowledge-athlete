...
class TestMemoryServiceParity:
    ...
    def test_surface_canonical_write_gate_failure_does_not_fallback_to_legacy_create(self, monkeypatch):
        service_mod = _load_memory_service(monkeypatch)
        ...
        with pytest.raises(HTTPException) as exc:
            service_mod.MemoryService(db_client=_FirestoreFake()).create_memory_for_surface(
                "uid-test",
                memory_db,
    ...
    def test_surface_canonical_create_uses_canonical_backend_without_rechecking_public_write(self, monkeypatch):
        service_mod = _load_memory_service(monkeypatch)
        memory_db = service_mod.MemoryDB.model_validate(_sample_memory_dict())
        create_memory = MagicMock()
        ...
        monkeypatch.setattr(
            ...
            "write",
            MagicMock(side_effect=AssertionError("surface canonical writes must not re-enter public write")),
        )
        ...
        with pytest.raises(HTTPException) as exc:
            service.create_memory_for_surface(
                "uid-test",
                memory_db,
                memory_system=MemorySystem.CANONICAL,
                consumer="omi_chat",
    ...
    def test_surface_canonical_edit_value_error_maps_to_404_without_legacy_fallback(self, monkeypatch):
        service_mod = _load_memory_service(monkeypatch)
        ...
        with pytest.raises(HTTPException) as exc:
            service.update_memory_content_for_surface(
                "uid-test",
                "missing-memory",
                "new content",
                memory_system=MemorySystem.CANONICAL,
    ...
    def test_canonical_backend_delegates_to_adapter(self, monkeypatch):
        ...
        with (
            ...
            patch(
                "utils.memory.memory_service.write_canonical_surface_memory",
                return_value="mem-1",
            ) as write_mock,
            patch(
    ...
    def test_surface_legacy_create_strips_canonical_lifecycle_fields(self, monkeypatch):
        service_mod = _load_memory_service(monkeypatch)
        ...
        result = service_mod.MemoryService(db_client=_FirestoreFake()).create_memory_for_surface(
            "uid-test",
            memory_db,
