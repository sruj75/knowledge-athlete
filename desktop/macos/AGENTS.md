...
Beta candidate tags are cut **on every macOS-affecting merge to `main`** (`push` trigger on the releasable desktop paths, plus a 15-min `schedule` backstop). Each run tags the newest retained macOS/release-control change since the last tag. A candidate reaches beta only after qualification passes:
