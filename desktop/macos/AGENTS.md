...
- **Workspace ownership.** `scripts/dev-instance.sh` resolves `omi-<worktree>` and nine Conductor ports; `OMI_HARNESS_PORT_OFFSET` preserves qualification allocation. Foreign listeners fail closed. `make desktop-run-local` records launch attempts and exact app token/PID/start/bundle/profile/bridge ownership; `make dev-status` checks it and `make dev-down` stops only recorded workspace processes, preserving history. Incomplete registration/stops retain evidence. Permission reopens keep the bundled port; successor recovery requires prior ownership, exact executable, source and bridge identity, never app-name adoption. Provider/admin secrets stay in the backend. Embedded `OMI_LOCAL_PROVIDER_MODE` follows the active stack: absent/invalid stays offline; explicit real AI still uses local accounts/storage.
