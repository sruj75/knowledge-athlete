...
function openAIToolDefinition({ exposedName, tool }) {
...
function generateExecutorsSwift() {
  ...
  // These commands are emitted only by the existing backend-owned onboarding
  ...
  const chatToolCases = swiftTools
    ...
    .concat(onboardingCommands.map((tool) => `    case ${swiftToolCaseName(tool.name)}`))
    .join("\n");

  const onboardingNames = onboardingCommands.flatMap((tool) => [tool.name, ...tool.aliases]);
  const onboardingDispatchCases = onboardingCommands
    .flatMap((tool) => [tool.name, ...tool.aliases].map((name) => `    case "${name}": return .${swiftToolCaseName(tool.name)}`))
    .join("\n");
...
${executorEntries}
  ...
  private static let backendOnboardingToolNames: Set<String> = ${JSON.stringify(onboardingNames)}
