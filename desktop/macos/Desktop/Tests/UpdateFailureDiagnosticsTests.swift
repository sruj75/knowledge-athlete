...
final class UpdateFailureDiagnosticsTests: XCTestCase {
  ...
  func testManualDownloadURLUsesConcreteStableEndpointByDefault() {
    ...
    XCTAssertEqual(
      AppBuild.manualDownloadURL(
        infoDictionary: releaseInfo, channel: "stable", isBetaIdentity: false)?.absoluteString,
  ...
  func testManualDownloadURLPreservesBetaChannel() {
    ...
    XCTAssertEqual(
      AppBuild.manualDownloadURL(
        infoDictionary: releaseInfo, channel: "beta", isBetaIdentity: false)?.absoluteString,
  ...
  func testClassifiesDeeplyWrappedSparkleDownloadFailureAsNetwork() {
    let url = URL(string: "https://updates.heyintentive.com/v2/desktop/appcast.xml")!
    let network = NSError(
    ...
    XCTAssertEqual(properties["failing_url_host"] as? String, "updates.heyintentive.com")
    ...
    XCTAssertEqual(properties["appcast_url_host"] as? String, "updates.heyintentive.com")
  ...
  func testAnalyticsPropertiesFallbackMessageIsNonEmpty() {
    let diagnostics = UpdateFailureDiagnostics(
      ...
      sourceAppVersion: "0.12.0",
      sourceAppBuild: "12000",
      appcastURLHost: "updates.heyintentive.com",
