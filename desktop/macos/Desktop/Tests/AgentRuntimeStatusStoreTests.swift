...
final class AgentRuntimeStatusStoreTests: XCTestCase {
  ...
  func testResultProjectionIndexesCanonicalSurfaceSessionAndRun() {
    ...
    let surface = AgentSurfaceReference.mainChat(chatId: "chat-1")
  ...
  func testUnknownResultTerminalStatusFailsClosed() {
    let store = AgentRuntimeStatusStore()
    let surface = AgentSurfaceReference.mainChat(chatId: "chat-unknown")
    let message = AgentRuntimeProcess.RuntimeMessage.parse(
  ...
  func testCancelledResultNeverProjectsSuccess() {
    let store = AgentRuntimeStatusStore()
    let surface = AgentSurfaceReference.mainChat(chatId: "chat-cancelled")
    let message = AgentRuntimeProcess.RuntimeMessage.parse(
  ...
  func testRestoresActiveMainChatRunFromKernelSnapshot() {
    let store = AgentRuntimeStatusStore()
    let surface = AgentSurfaceReference.mainChat(chatId: "chat-restore")
    ...
    let projection = store.projection(for: surface)
    XCTAssertEqual(projection?.sessionId, "sess-restored")
    XCTAssertEqual(projection?.runId, "run-restored")
    XCTAssertEqual(projection?.status, .running)
  ...
  func testUpdateActivityDoesNotReviveTerminalProjection() {
    let store = AgentRuntimeStatusStore()
    let surface = AgentSurfaceReference.mainChat(chatId: "chat-terminal")
    let message = AgentRuntimeProcess.RuntimeMessage.parse(
