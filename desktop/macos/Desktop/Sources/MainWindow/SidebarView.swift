        ...
        && monitoringAutoRestartAttempts < maxAutoRestartAttempts
      {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
          ...
          if plugin.hasScreenRecordingPermission {
            log(
              "SidebarView: Auto-restarting monitoring (attempt \(attempt)/\(maxAutoRestartAttempts))"
            )
...
struct NavItemWithStatusView: View {
  ...
  var body: some View {
    ...
    .onTapGesture {
      log(
        "SIDEBAR: NavItemWithStatus '\(label)' row tapped at mouse position: \(NSEvent.mouseLocation)"
      )
      onTap()
    }
    .onHover { hovering in
...
private struct SidebarAudioBar: View {
  ...
  private var barHeight: CGFloat {
    ...
    let centerOffset =
      abs(CGFloat(index) - CGFloat(totalBars - 1) / 2.0) / (CGFloat(totalBars) / 2.0)
