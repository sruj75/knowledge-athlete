...
class TierManager {
  ...
  private func computeTierLevel() async -> Int {
    do {
      ...
      log(
        "TierManager: Stats - memories=\(ms.total), tasks=\(taskCount), conversations=\(conversations), screenshots=\(screenshotCount)"
      )
      ...
      guard conversations >= tier5ConversationsThreshold && screenshotCount >= tier5ScreenshotsThreshold else {
        return tier
      }
      tier = 5
...
// MARK: - Helpers
extension Int {
  fileprivate func clamped(min: Int) -> Int {
