...
actor RewindIndexer {
  ...
  private func ensureInitialized() async -> Bool {
    ...
    do {
      ...
      if initFailureCount <= 3 {
        logError(
          "RewindIndexer: Failed to initialize (attempt \(initFailureCount), next retry in \(Int(backoffSeconds))s): \(error)"
        )
      } else if initFailureCount % 10 == 0 {
        log(
          "RewindIndexer: Still failing to initialize (attempt \(initFailureCount), next retry in \(Int(backoffSeconds))s)"
        )
      }
      return false
    }
  ...
  private func recordOCROutcome(_ outcome: OCROutcome) {
    ...
    if now.timeIntervalSince(statsLastLogTime) >= statsLogInterval {
      let parts = [
        "\(statsTotalFrames) frames", "\(statsOCRRan) OCR'd", "\(statsSkippedFrequency) skipped (frequency)",
        "\(statsSkippedDedup) skipped (dedup)",
      ]
  ...
  private static func parseLegacyChunkTimestamp(_ filename: String) -> Date? {
    ...
    guard filename.hasPrefix("chunk_"),
      ...
      filename.count == 26
    else { return nil }  // "chunk_" (6) + 8 + "_" (1) + 6 + ".hevc" (5) = 26
