...
actor RewindDatabase {
  ...
  private func migrate(_ queue: DatabasePool) throws {
    ...
    migrator.registerMigration("addVideoChunkColumns") { db in
      ...
      try db.create(
        index: "idx_screenshots_videoChunkPath",
    ...
    migrator.registerMigration("createNormalizedOCR") { db in
      ...
      try db.create(
        index: "idx_ocr_occurrences_screenshot",
        on: "ocr_occurrences", columns: ["screenshotId"])
      try db.create(
        index: "idx_ocr_occurrences_text",
    ...
    migrator.registerMigration("expandTranscriptionSchema") { db in
      ...
      try db.create(
        index: "idx_sessions_conversationStatus", on: "transcription_sessions", columns: ["conversationStatus"])
    ...
    migrator.registerMigration("createTaskDedupLog") { db in
      ...
      try db.create(
        index: "idx_dedup_log_deleted_at",
    ...
    migrator.registerMigration("createTaskChatMessages") { db in
      ...
      try db.create(
        index: "idx_task_chat_messages_messageId", on: "task_chat_messages", columns: ["messageId"], unique: true)
      try db.create(
        index: "idx_task_chat_messages_created", on: "task_chat_messages", columns: ["taskId", "createdAt"])
  ...
  func reduceOCRDataPrecisionIfNeeded() async {
    ...
    do {
      ...
      log(
        "RewindDatabase: OCR precision reduction complete — processed \(totalProcessed) rows, updated \(totalUpdated)")
    } catch {
