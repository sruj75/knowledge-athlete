...
actor InsightAssistant: ProactiveAssistant {
  ...
  private static func compressForGemini(_ data: Data) -> Data? {
    ...
    guard let dest = CGImageDestinationCreateWithData(mutableData as CFMutableData, "public.jpeg" as CFString, 1, nil)
    else { return nil }
  ...
  private func runAdviceExtraction(
    ...
    phase1Loop: for iteration in 0..<7 {
      ...
      case "request_screenshot":
        ...
        log(
          "Insight: P1 request_screenshot iter \(iteration): id=\(chosenScreenshotId ?? 0), findings=\(findings.prefix(200))"
        )
        break  // Exit phase 1
  ...
  private func buildPhase1Tools() -> GeminiTool {
    GeminiTool(functionDeclarations: [
      ...
      GeminiTool.FunctionDeclaration(
        ...
        description:
          "Request to view a specific screenshot. Call this when you've found something interesting via SQL and want to see the actual screen. Provide the screenshot ID and a summary of your findings so far. The screenshot will be shown to you for final analysis.",
        parameters: GeminiTool.FunctionDeclaration.Parameters(
          type: "object",
          properties: [
  ...
  private func buildPhase2Tools() -> GeminiTool {
    GeminiTool(functionDeclarations: [
      GeminiTool.FunctionDeclaration(
        name: "execute_sql",
        description:
          "Cross-reference your findings by querying the database. Use this to check if an issue was resolved in later screenshots, verify context across time, or look up related activity. The screenshots table has: id INTEGER, timestamp TEXT, appName TEXT, windowTitle TEXT, ocrText TEXT, focusStatus TEXT. SELECT queries only.",
        parameters: GeminiTool.FunctionDeclaration.Parameters(
          type: "object",
          properties: [
            "query": .init(type: "string", description: "SQL SELECT query to execute on the screenshots table")
      ...
      GeminiTool.FunctionDeclaration(
        ...
        parameters: GeminiTool.FunctionDeclaration.Parameters(
          ...
          properties: [
            ...
            "category": .init(
              type: "string", description: "Category of insight",
              enumValues: ["productivity", "communication", "learning", "other"]),
          ...
          required: [
            "advice", "headline", "category", "source_app", "confidence", "context_summary", "current_activity",
          ]
        )
      ),
      GeminiTool.FunctionDeclaration(
...
private func withThrowingTimeout<T: Sendable>(seconds: Double, operation: @escaping @Sendable () async throws -> T)
  async throws -> T
{
