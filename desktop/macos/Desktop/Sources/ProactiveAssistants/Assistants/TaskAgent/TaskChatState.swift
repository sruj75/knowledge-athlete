...
struct TaskChatProducingRunProjection: Equatable {
  ...
  mutating func bindResult(
    ...
    guard self.assistantMessageID == assistantMessageID,
      ...
      let normalizedAttemptID = Self.normalized(attemptID)
    else { return false }
  ...
  func terminalMessageID(for projection: AgentRunProjection) -> String? {
    guard projection.status.isTerminal,
      ...
      Self.normalized(projection.attemptId) == attemptID
    else { return nil }
...
class TaskChatState: ObservableObject {
  ...
  init(
    ...
    self.boundAuthorizationSnapshot =
    ...
    self.attachJournalEventsOperation =
      attachJournalEventsOperation ?? {
    ...
    self.listJournalTurnsOperation =
      listJournalTurnsOperation ?? {
    ...
    self.recordJournalExchangeOperation =
      recordJournalExchangeOperation ?? {
    ...
    repeat {
      ...
      do {
        ...
        while fetchNextPage {
          ...
          {
            log(
              "TaskChatState[\(workstreamId)]: journal gap (expected=\(journalHighWater + 1), got=\(firstUnapplied.turnSeq))"
            )
            fetchNextPage = false
          }
    ...
    runtimeProjectionCancellable = AgentRuntimeStatusStore.shared.$projectionsBySurface
      ...
      .sink { [weak self] projections in
        guard let self, self.hasCurrentOwner,
          let projection = projections[surface.key]
        else { return }
