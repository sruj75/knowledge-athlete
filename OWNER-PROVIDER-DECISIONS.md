...
- Created and verified 2026-08-29: public-ingress Cloud Run service `knowledge-athlete-dev` in `knowledge-athlete/us-west1`.
  ...
  - Permanent-free bootstrap shape: zero minimum instances, one maximum instance, request-based CPU, 1 vCPU, 2 GiB memory, billing mode disabled, Vertex AI disabled, public egress to the external Upstash free database, and no Google Memorystore. The checked-in development runtime/foundation manifest now declares this same cost-bounded topology; this does not claim that the active revision contains later provider changes or that the service is production-ready.
...
- Provisioned 2026-09-02: the development deployment foundation for the existing `knowledge-athlete-dev` service.
  ...
  - `intentive-dev-deploy@knowledge-athlete.iam.gserviceaccount.com` owns development Cloud Run deploy/traffic, Artifact Registry writes, Cloud Tasks administration, Secret Manager metadata inspection without secret-value access, and act-as permission only on the development runtime identity.
...
- Langfuse now owns Chat tracing and prompt management. Verified 2026-09-02: the existing local credentials authenticate to the owned US Cloud project `Intentive`; `intentive-chat-system` resolves to blank version 1 with `production` and `latest`, while `intentive-runtime-bundle` history remains untouched. The public and secret credentials are stored separately as exact Secret Manager version 1 values, GitHub environment `development` selects both version 1 values, and only the limited development runtime identity can read them. The active Cloud Run revision does not bind them yet; a real fail-open runtime trace remains required after the next complete development deployment.
...
- [x] Store the development Redis password, Firebase API key, and Google OAuth client ID/secret as exact Secret Manager version 1 values, plus the Gemini authorization key as exact version 2, the OpenAI TTS-only key as exact version 1, and both Langfuse credentials as exact version 1 values. The runtime identity has secret-level access only to `REDIS_DB_PASSWORD`, `FIREBASE_API_KEY`, `GOOGLE_CLIENT_SECRET`, `GEMINI_API_KEY`, `OPENAI_API_KEY`, `LANGFUSE_PUBLIC_KEY`, and `LANGFUSE_SECRET_KEY`; the client ID remains non-secret deployment configuration.
...
- [ ] OpenAI TTS-only key version 1, Gemini authorization key version 2, and both Langfuse version 1 credentials are prepared, provider-verified, and selected by the development GitHub environment. Bind them only as part of the next complete development deployment, then prove the authenticated backend TTS route, a real Gemini turn, and one real fail-open Langfuse trace. Do not create Anthropic or Artificial Analysis credentials. Modulate remains deliberately postponed. No provider secret may be invented, committed, or copied from Omi.
