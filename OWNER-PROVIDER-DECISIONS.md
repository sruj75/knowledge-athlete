...
- Created and verified 2026-08-29: public-ingress Cloud Run service `knowledge-athlete-dev` in `knowledge-athlete/us-west1`.
  ...
  - Active revision as of 2026-09-04: `knowledge-athlete-dev-652be3e-33860199870-1`, serving 100% of development traffic from owned immutable OCI image digest `sha256:33b022310d47ba6a8eda9991ff6448f6672e07459917b1f31317cb9a6c594474` (runtime linux/amd64 manifest `sha256:5f0c64942a1818db118a3ab8767674dd8039118918c4bb00b1226e93a7fe5bd4`), built from exact `main` commit `652be3e634deaf101ca767bd2a5965e4fc621c80`.
  ...
  - Live probing first caught that the inherited OAuth route's `BASE_API_URL` was missing from the hosted runtime. PR #65 now renders it from the environment-owned canonical Cloud Run URL. The active revision passed a real `/v1/auth/authorize` infrastructure probe: Redis accepted the PKCE-bound one-time session and the backend returned `307` to `accounts.google.com` with callback `https://knowledge-athlete-dev-674306938907.us-west1.run.app/v1/auth/callback/google`. No user completed an interactive sign-in during this setup probe.
...
- Provisioned 2026-09-02: the development deployment foundation for the existing `knowledge-athlete-dev` service.
  ...
  - Verified 2026-09-04: automatic development deployment run `33860199870` attempt 1 exchanged GitHub OIDC for the restricted deploy identity, built and verified the immutable image from exact `main`, created a zero-traffic candidate, minted and deleted its short-lived Firebase probe user, passed public health, authenticated two-turn Gemini Chat, Firestore read, managed Gemini realtime, exact composition, and serving-vector checks, then promoted the candidate to 100% traffic. Rollback was not needed. Independent live probes returned health `200`, protected Chat without Firebase auth `401`, and Google authorization `307` with the canonical owned callback.
...
- `MODULATE_API_KEY` is retained for fixed managed live and prerecorded-overflow STT. Verified 2026-09-02: the owned Modulate key `Intentive development` is limited to 500 credits and only `velma-2-stt-streaming` plus `velma-2-stt-batch`. Its value is stored only as enabled Secret Manager version 1, GitHub environment `development` selects exact version `1`, and only the limited development runtime identity can read it. The active Cloud Run revision now binds version 1. Paid live batch/streaming continuity remains an S-31 proof and was not exercised by the development deployment gate.
...
- [ ] Prove the active revision's authenticated OpenAI TTS route and real Modulate batch/streaming continuity. These are live provider checks, not reasons to add alternate providers or rework the deployment topology.
