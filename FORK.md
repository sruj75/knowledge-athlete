...
`desktop/macos/run.sh --yolo` skips the local backend and selects the owned private
`knowledge-athlete-dev` Cloud Run URL. It still requires an explicitly supplied owned
Firebase API key, and the desktop cannot directly invoke the private service until its
authentication boundary is configured. Use the emulator path for normal development.
...
| Google Cloud project | existing `agentic-accountability`; no separate MVP dev/prod projects |
| Development Cloud Run service | `knowledge-athlete-dev` in `us-west1` |
| Container repository | existing `us-west1` Artifact Registry repository `intentive` |
...
- The private development Cloud Run service and deny-all Firestore database exist, but
  Firebase app registrations, Firebase/Auth IAM, Redis, and direct desktop invocation
...
| Google OAuth/Firebase app registrations | Inherited production client/app remain in `GoogleService-Info.plist`; owned development app `1:674306938907:ios:befed665f1aa0cd09b40be` is registered for `com.heyintentive.intentive.dev`; the local emulator app remains synthetic. Google OAuth client fields are pending owned provider enablement. | `desktop/macos/Desktop/Sources/GoogleService-Info.plist`, `desktop/macos/Desktop/Sources/GoogleService-Info-Dev.plist`, `desktop/macos/Desktop/Sources/GoogleService-Info-Local.plist` | Development now selects the owned Firebase app, but hosted Google sign-in is not ready until its provider/client registration exists. Production-family builds remain fail-closed work and must not consume the inherited plist. | Intentive development; Omi/BasedHardware production residue; local harness; Google | external identifier |
