...
| GCP/Firebase projects | Production `based-hardware`; development `based-hardware-dev`; Omi-named Cloud Run, service-account, image, and secret resources | `.github/workflows/gcp_backend*.yml`, `backend/deploy/runtime_env.yaml`, `infrastructure/opentofu/` | Deployment and runtime identities target inherited Omi projects and cloud principals until S-27 establishes verified ownership. | Omi/BasedHardware; Google Cloud | external identifier; release infrastructure |
| Canonical backend deployment identity | The repository declares one `backend` Cloud Run service/image per environment using `gcr.io/based-hardware[-dev]/backend:<immutable-tag>` | `backend/deploy/runtime_env.yaml`, `backend/runtime_images.json`, `.github/workflows/gcp_backend.yml`, `.github/workflows/gcp_backend_auto_dev.yml` | Source closure, runtime env, candidate acceptance, traffic, and rollback now have one repository owner. S-27 must inventory any inherited live `desktop-backend` resources; S-26 does not claim they are absent or safe to delete. | Omi/BasedHardware deployment; Google Cloud | external identifier; release infrastructure |
