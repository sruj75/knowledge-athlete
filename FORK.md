...
| Google OAuth/Firebase app registrations | Owned development app `1:674306938907:ios:befed665f1aa0cd09b40be` is registered for `com.heyintentive.intentive.dev`; Google sign-in is enabled and its generated OAuth client is tracked in the development plist; the local emulator app remains synthetic. | `desktop/macos/Desktop/Sources/GoogleService-Info-Dev.plist`, `desktop/macos/Desktop/Sources/GoogleService-Info-Local.plist` | Development selects the owned Firebase app and hosted Google provider. Release builds require an owned environment-supplied plist and fail closed without one. | Intentive development; local harness; Google | external identifier |
| Firebase apps | Owned development project `knowledge-athlete`, app `com.heyintentive.intentive.dev`; local project `demo-heyintentive-local` is emulator-only. | `desktop/macos/Desktop/Sources/GoogleService-Info-Dev.plist`, `desktop/macos/Desktop/Sources/GoogleService-Info-Local.plist` | Canonical development builds receive the owned project configuration; local profiles stay on emulators. Beta/Stable consume their separately approved protected configurations during the release build. | Intentive development; local harness | service endpoint; external identifier |
