...
| Modulate reproduction script | No CI, release, test-runner, or production caller | inherited `omi-pr-assets` download and an unusable caller-supplied fixture path | retired with its README and Pyright enrollment; the owned release probe remains authoritative | S-03 existing-contract rule; caller proof | Deleted, C10 follow-up | repository caller search; release-probe workflow contract |
...
| `backend/scripts/stt/modulate_repro/test_audio.wav` | No retained caller or artifact | No fixture is required because the orphan reproduction script is retired; the separately owned and licensed `backend/testing/release_fixtures/transcription-release-probe.wav` remains the release-authority fixture |
