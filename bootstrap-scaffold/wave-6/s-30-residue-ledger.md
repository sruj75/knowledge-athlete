...
| task source display and Chat schema annotation | Current task UI, Gemini schema context, and analytics | caller-free `transcription:omi` label/icon and `omi` origin instruction | legacy stored value projects as neutral `task` at every external boundary; model schema retains current `conversation` plus supported local sources | S-30 current rendered/model identity | Removed/neutralized, C10 | task metadata and Chat discoverability tests |
...
| Desktop tests | Final focused identity/truth suite: 43 passed; strict changed-source flow coverage passes 131/131. The component runner's launcher, packaging, release, E2E, and embedded-backend stages pass. Its isolated Swift lane reports 393/402 suites green; nine inherited suites fail in unchanged boundaries: seven abort when the XCTest host reaches the fail-closed production URL rule, and `RewindRetentionCleanupTests` / `RewindStorageVideoFrameExtractionTests` retain stale pre-S-28 storage expectations. The failed test files and routing/storage owners have no S-30 diff. |
