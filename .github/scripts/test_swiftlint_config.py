...
BASELINE_PREPARER_PATH = REPO_ROOT / "desktop/macos/scripts/prepare-swiftlint-baseline.py"
...
class SwiftLintConfigTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        spec = importlib.util.spec_from_file_location("prepare_swiftlint_baseline", BASELINE_PREPARER_PATH)
        assert spec and spec.loader
        cls.baseline_preparer = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(cls.baseline_preparer)
    ...
    def test_baseline_preparer_rebases_file_urls_to_the_current_checkout(self):
        source_url = "file:///Users/runner/work/omi/omi/desktop/macos/Desktop/Sources/Example.swift"
        fixture = [{"violation": {"location": {"file": source_url, "line": 12}}}]
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            input_path = root / "baseline.json"
            output_path = root / "prepared.json"
            desktop_dir = root / "checkout/Desktop"
            input_path.write_text(json.dumps(fixture), encoding="utf-8")

            self.baseline_preparer.rebase_baseline(input_path, desktop_dir, output_path)

            prepared = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(
                prepared[0]["violation"]["location"]["file"],
                (desktop_dir.resolve() / "Sources/Example.swift").as_uri(),
            )

    def test_baseline_preparer_rejects_a_location_outside_desktop(self):
        fixture = [{"violation": {"location": {"file": "file:///tmp/Example.swift", "line": 12}}}]
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            input_path = root / "baseline.json"
            input_path.write_text(json.dumps(fixture), encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "not beneath Desktop"):
                self.baseline_preparer.rebase_baseline(input_path, root / "Desktop", root / "prepared.json")
    ...
    def test_pinned_runner_is_the_macos_manifest_producer(self):
        ...
        self.assertIn('BASELINE_PREPARER="$SCRIPT_DIR/prepare-swiftlint-baseline.py"', wrapper)
        self.assertIn('--baseline "$local_baseline_dir/baseline.json"', wrapper)
