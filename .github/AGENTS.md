...
- Development backend acceptance must stage `GCP_SERVICE_ACCOUNT` only in a mode-0600 runner file for Firebase probe signing, validate that its project matches `FIREBASE_AUTH_PROJECT_ID`, and delete it after the probe; never copy it into the image or deploy it as runtime configuration.
